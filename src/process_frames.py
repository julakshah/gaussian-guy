...
def main(path: str):
    """
    Main function to read image positions from their names
    Our arm movement script writes the x,y,z,roll,pitch,yaw into the name of the image
    We can extract that fairly easily with regular expressions, divide the dataset into two parts
    for testing versus training, and write the results to a json file

    Args:
        path (str): file path to the directory of images from the scan
    """
    cwd = os.getcwd()
    ...
    # Automate COLMAP ---- not currently used!
    # My PyColmap isn't working, and since it's deprecated in newer versions, it seems better
    # to just use the command line instead
    # run_sfm(path,path)
